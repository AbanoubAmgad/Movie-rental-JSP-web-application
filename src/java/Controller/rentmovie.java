    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                RS = Stmt.executeQuery("SELECT * FROM cart where userID = '" + email + "' and movieID = '" + name + "';");
                boolean empty = true;
                while (RS.next()) {
                    empty = false;
                }
                if (empty) {
                    ...
                    Stmt.executeUpdate("Insert into cart Values ('" + email + "','" + name + "','" + 7 + "')");
                    RS.close();
                    Stmt.close();
                    Con.close();
                    ...
                    out.println("You Already Rented that movie click ");
                        out.println("<a href=\"UserHome.jsp\"> Here </a>");
                        out.println(" to get back");
                        RS.close();
                        Stmt.close();
                        Con.close();
                }
